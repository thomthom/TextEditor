module TT_Writer
    ...
    # Menus
    m = UI.menu( 'Draw' )
    m.add_item( '3d Notepad' ) { self.writer_tool }
  ...
  class TextEditorTool
  	...
  	def open_ui
        ...
        :resizable => false
  		...
  		txtInput = TT::GUI::Textbox.new( @text.gsub( "\n", '\n' ) )
  		txtInput.top = 5
      txtInput.left = 5
      txtInput.width = 200
  	...
  	def input_changed( value )
  		...
  		@group.set_attribute( 'TT_Editor', 'Text',   @text )
  		@group.set_attribute( 'TT_Editor', 'Bold',   @bold )
  		@group.set_attribute( 'TT_Editor', 'Italic', @italic )
  		@group.set_attribute( 'TT_Editor', 'Size',   @size )
  		@group.set_attribute( 'TT_Editor', 'Extrude',   @extrude )
  		@group.set_attribute( 'TT_Editor', 'Align',  @align )
  	end
